...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    ...
    if (password === 'Loki2024!') {
      // Check if we already have an API key stored from previous session
  ...
  const handleApiKeySubmit = (e: React.FormEvent) => {
    ...
    if (apiKey.trim().length > 10) {
      // Store the key locally on the device as requested
            ...
            <form onSubmit={handleApiKeySubmit} className="space-y-4 animate-fade-in">
                <div className="relative">
                  ...
                  <input
                      ...
                      autoFocus
                  />
                  <p className="text-[10px] text-gray-400 mt-2">Key is stored securely on this device.</p>
                </div>
